...
const isString = () => {
    return (props.type.match(/(char)|(blob)|(binary)/g) || []).length > 0
}

const isText = () => {
    return (props.type.match(/(text)/g) || []).length > 0
}

const isNumber = () => {
    return (props.type.match(/(int)|(float)|(double)|(dec)|(bit)/g) || []).length > 0
}

const isBoolean = () => {
    return (props.type.match(/(bool)/g) || []).length > 0
}

const isEnum = () => {
    return (props.type.match(/(enum)|(set)/g) || []).length > 0
}

const isDate = () => {
    return (props.type.match(/(date)|(time)/g) || []).length > 0
}

const isJson = () => {
    return (props.type.match(/(json)/g) || []).length > 0
}
...
<template>
    <div class="type-indic">
        ...
        <span v-else-if="isEnum() === true">
            <svg width="12" height="12" viewBox="0 0 512 512" fill="#000000" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve">
                <g>
                    <g>
                        <g>
        ...
        <span v-else-if="isDate() === true">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M3 10H21M7 3V5M17 3V5M6.2 21H17.8C18.9201 21 19.4802 21 19.908 20.782C20.2843 20.5903 20.5903 20.2843 20.782 19.908C21 19.4802 21 18.9201 21 17.8V8.2C21 7.07989 21 6.51984 20.782 6.09202C20.5903 5.71569 20.2843 5.40973 19.908 5.21799C19.4802 5 18.9201 5 17.8 5H6.2C5.0799 5 4.51984 5 4.09202 5.21799C3.71569 5.40973 3.40973 5.71569 3.21799 6.09202C3 6.51984 3 7.07989 3 8.2V17.8C3 18.9201 3 19.4802 3.21799 19.908C3.40973 20.2843 3.71569 20.5903 4.09202 20.782C4.51984 21 5.07989 21 6.2 21Z" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
        </span>

        <span  v-else-if="isJson() === true">
            <svg width="12" height="12" viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg" fill="#000000">
